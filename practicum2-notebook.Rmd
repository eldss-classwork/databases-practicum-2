...
<!-- Need an explanation of what is going on here. -->
...
SELECT parentTconst, primaryTitle, MAX(seasonNumber) AS numSeasons
FROM Episodes 
JOIN 
...
```{sql connection=conn, output.var="frequency"}
...
FROM
  ...
  JOIN 
    ...
    WHERE format_id=5) series 
  ON series.tconst=Episodes.parentTconst
  GROUP BY parentTconst) seasonCts
GROUP BY numSeasons;
```

```{r, message='hide'}
# print(seasons)
seasons <- drop_na(seasons)
seasonCount <- seasons$numSeasons
hist(seasonCount, main = "Season Frequency", xlab = "Number of Seasons", breaks=87, col = "blue", xlim = range(pretty(c(0,300))), ylim = range(pretty(c(0,68000))))
```
...
hist(seasonCount, main = "Season Frequency", xlab = "Number of Seasons", col = "blue", ylim = range(pretty(c(0,69000))), breaks = 65)
```

```{r}
hist(seasonCount, main = "Season Frequency", xlab = "Number of Seasons", col = "blue", ylim = range(pretty(c(0,69000))), breaks = c(seq(1,300,5)))
```

```{r, message='hide'}
hist(seasonCount, main = "Season Frequency", xlab = "Number of Seasons", col = "blue", ylim = range(pretty(c(0,69000))), breaks = c(seq(1,50,1), seq(50,300,50)), prob = FALSE)
```
