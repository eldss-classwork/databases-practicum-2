...
writeTsvFilterByTitle <- function(dataframe, filterSet, filename) {
  #' Writes an existing dataframe after filtering it on tconst
  
  # This may take several minutes...
  filter(dataframe, has.key(tconst, filterSet))
  
  # Write out the data
  paths <- getFullAndSamplePaths(outPath)
  write_tst(dataframe, paths$sample)
}
...
# paths <- getOldAndNewPaths(nameBasics)
# df <- createDataframe(paths$full, 50)
# kfTitles <- drop_na(df["knownForTitles"]) %>%
#     mutate(knownForTitles = strsplit(as.character(knownForTitles), ",")) %>%
#     unnest(knownForTitles)
# head(kfTitles)
```
