...
#       May not work, doesn't look like there is an encoding parameter.
...
Getting data from the name basics data is more complicated. Unlike the other files, there is no natural ordering on the media IDs.

```{r}
df <- read_tsv(nameBasics)
head(df)
```

```{r}
df.1 <- read_tsv(titleAkas, na=c("\\N"))
head(df.1)
```
