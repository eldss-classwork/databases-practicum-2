...
# Add to database
dbWriteTable(conn, "Professions", df.professions, overwrite=TRUE)
dbWriteTable(conn, "Primary_Profession", df.primaryProfessions, overwrite=TRUE)
...
# Transfer the new dataframes into the database tables
# Overwrite is true because this should be the initial reading of the data
dbWriteTable(conn,"Characters",df_characters, overwrite=TRUE)
...
```{r, message=FALSE}
# Get aka types
df.akaTypes <- df.akas[c("titleId", "ordering", "types")]
...
# Add to database
dbWriteTable(conn, "Types", df.types, overwrite=TRUE)
...
```{r, message=FALSE}
# Get aka attributes
df.akaAtts <- df.akas[c("titleId", "ordering", "attributes")]
...
# Add to database
dbWriteTable(conn, "Attributes", df.attributes, overwrite=TRUE)
